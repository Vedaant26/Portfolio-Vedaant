export const projectData = [
    {
        image: '/work/insta-ai.png',
        category: 'next js',
        name: 'FaceAuth',
        description:
            'FaceAuth is an AI-powered facial recognition application which can be run both online and in the offline mode that provides secure and seamless authentication for various platforms.',
        link: 'https://face-auth-nhai.vercel.app/',
        github: 'https://github.com/Vedaant26/FaceAuth',
    },
    {
        image: '/work/static.png',
        category: 'next js',
        name: 'Static',
        description:
            'Static is a no-code website builder that allows users to create and publish SEO-optimized static sites effortlessly',
        link: 'https://static-akb.vercel.app',
        github: 'https://github.com/Vedaant26/FaceAuth',
    },
];
